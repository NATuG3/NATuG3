...
\usepackage{xcolor}
\usepackage{amsmath}
\usepackage{tcolorbox} % extends colorbox package so that we can round the corners of colorboxes
...
% define constants
\def\fileExtension{nano}

% define colors and styles
\definecolor{sunken-color-background}{rgb}{0.94,0.94,0.94}
\definecolor{sunken-color-outline}{rgb}{0.9,0.9,0.9}

% styles to make a block of text appear more sunken
...
% all graphics will come from the resources/images folder
...
\begin{document}
	...
	\begin{enumerate} \label{sect:getting-natug-running}
		\item Visit \href{Python's download page}{www.python.org/downloads}, and then install the most recent version of Python for your operating system. NATuG has been confirmed to work on versions up to Python \sunken{3.11.1}.
		...
		\begin{enumerate}
			\item \sunken{cd $<$filepath$>$} to enter the directory of the project. \label{enum:enter-directory}
			\item \sunken{python~-m~venv~venv} to create a virtual environment for the needed libraries to go into.
			\item \sunken{venv/Scripts/activate} if you are on windows, or “source~myenv/bin/activate” if you are on Mac/Linux, to enter into the virtual environment. You will know that you have successfully entered the virtual environment if the current line in terminal begins with “(venv).” \label{enum:activate-venv}
			\item \sunken{python -m pip install -r requirements.txt} to automatically install all the needed libraries. They may take a while to download.
			\item \sunken{python -m launcher} to run the program. The first launch may take a bit while the code compiles. \label{enum:run-program}
		\end{enumerate}
	...
	Constructing a DNA nanotube is a complex, multi-stage process, but NATuG streamlines the steps. However, NATuG also provides a high level of customizability and advanced features that can feel overwhelming at first. Here, you will find a list of things to do that should help you gain a basic level familiarity with NATuG. Each step in this section is designed to provide an exploration opportunity for NATuG's various a core features, in order of how you would generally go about designing a DNA nanotube.
	\linebreak
	\linebreak
	...
	\begin{figure}[h]
		\centering
		\includegraphics[width=1.5in]{file-menu.png}
		\caption{The file menu}
		\subcaption{In the screenshot view submenu is expanded.}
		\label{fig:file-menu}
	\end{figure}
	
	\subsubsection{Breakdown} \label{sect:file-menu-breakdown}
	Herein lies a breakdown of what the various buttons of the File Menu do, and their respective keyboard shortcuts (if they have any).
	
	\paragraph{File} 
	The file submenu contains functions pertaining to file and save management. 
	
	\subparagraph{Open} (\sunken{control+o})
	Open a save. This will create a dialog prompting you to choose a file ending with \sunken{.\fileExtension} that is a save created via the File Menu's "save" function. 
	
	\subparagraph{Save} (\sunken{control+s})
	Create a save of the current state of the program. This will create a dialog prompting you to choose a location and a file name to create a save at. If you enter a file name that has an extension of anything other than \sunken{.\fileExtension}, or you do not specify a file extension, \sunken{.\fileExtension} will always be the file extension. 
	
	\paragraph{View}
	The view submenu contains functions partaining to the user interface itself.
	\subparagraph{Config}
	Hide the Config Panel if it is currently being displayed, or display it if it is currently hidden.
	
	\subparagraph{Helices Top View}
	Hide the Top View Plot if it is currently being displayed, or display it if it is currently hidden.
	
	\subparagraph{Recolor Strands}
	Rerun the strand coloring algorithm. For more information on what this entails, see section~\ref{sect:side-view-plot-styling}.
	...
	\begin{center}
		\begin{tabular}{|p{1in}|p{.7in}|p{3.5in}|}
			...
			Right Count & integer (x3) & A set of three integers representing how many NEMids to generate for the other helix in the double helix. See above for further descriptions of the boxes. \\
		\end{tabular}
	\end{center}

	\subsubsection{Presets}
	
	The Domains Tab allows you to export and sets of domain settings, along with the complementary symmetry settings, in a matter of clicks. This section covers how to save and import collections of domains.
	
	\paragraph{Exporting}
	To save the current state of the Domains Tab, including the Subunit Domains and symmetry settings, click the save button (the floppy disk icon), and choose a location to save. A file dialog will prompt you to choose an output filepath and file name for the save, and will open by default to the filepath \sunken{saves/domains/presets}. The output file is a .csv file with a format outlined in section~\ref{sect:domains-export-csv-format}.
	
	\paragraph{Importing}
	To import the current state of the Domains Tab, including the Subunit Domains and symmetry settings, click the load button (the arrow pointing down into a box). A file dialog will prompt you to choose an output filepath and file name for the profile that you are trying to import, and will open by default to the filepath \sunken{saves/domains/presets}. If the file is of the wrong format, and error will be raised. 
	
	NATuG includes, by default, within the \sunken{saves/domains/presets} folder, various different profiles, listed below. For the most part, the names are an indication of the top view nanotube shape.
	\begin{itemize}
		\item circle.csv
		\item heart.csv
		\item hexagon.csv
