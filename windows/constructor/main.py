...
class window(QMainWindow):
        ...
        try:
            ...
            self.docked_widgets.top_view.topLevelChanged.connect(
                lambda: helpers.unrestrict_scale_upon_float(
                    self.docked_widgets.top_view, restored_func=self.resizeEvent
                )
            )
    ...
    def resizeEvent(self, resizeEvent):
        ...
        # top view resizing
        #
        # maximum width = window_width/3.5
        # minimum width = half of the maximum width
        if not self.docked_widgets.top_view.isFloating():
            new_width = int(2 * window_width / 7)
            self.docked_widgets.top_view.setMaximumWidth(new_width)
            self.docked_widgets.top_view.setMinimumWidth(int(new_width / 2))

        # config resizing
        #
        # maximum width = window_width/3.5
        self.config.setMaximumWidth(int(2 * window_width / 7))
