...
class Table(QTableWidget):
    ...
    def __init__(self, parent) -> None:
        ...
        self.dump_domains(references.domains.current.subunit.domains)

        # when widget value is changed run fetch_domains and update current settings
        @self.cell_widget_updated.connect
        def _():
            references.domains.current.subunit.domains = self.fetch_domains()
    ...
    def dump_domains(self, domains: list) -> None:
        ...
        for index, domain in enumerate(domains):
            ...
            # append to the references row storage container
